...
public class DeleteContractByIdCommandHandlerTests
{
    ...
    public async Task Handle_ShouldReturnFailureResponse_WhenContractDoesNotExist()
    {
        ...
        _contractRepository.FindByIdAsync(command.ContractId).Returns(Result<Contract>.Failure("Contract does not exist."));
        var handler = new DeleteContractByIdCommandHandler(_contractRepository);
        ...
        Assert.False(result.IsSuccess);
        Assert.Equal("Failed to delete contract.", result.Message);
        Assert.Contains($"Contract with Id {command.ContractId} does not exist.", result.ValidationsErrors!["ContractId"]);
    }
    ...
    public async Task Handle_ShouldReturnFailureResponse_WhenContractRepositoryDeleteByIdAsyncFails()
    {
        ...
        _contractRepository.FindByIdAsync(contractResult.Value.Id).Returns(Result<Contract>.Success(contractResult.Value));
        _contractRepository.DeleteByIdAsync(contractResult.Value.Id).Returns(Result.Failure("Error deleting contract."));
