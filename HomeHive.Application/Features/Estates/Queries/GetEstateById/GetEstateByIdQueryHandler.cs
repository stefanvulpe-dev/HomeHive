...
public class GetEstateByIdQueryHandler : IQueryHandler<GetEstateByIdQuery, GetEstateByIdResponse>
{
    ...
    public async Task<GetEstateByIdResponse> Handle(GetEstateByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            Success = true,
            Estate = new EstateDto(estateResult.Value.OwnerId, estateResult.Value.EstateType.ToString(), estateResult.Value.EstateCategory.ToString(), estateResult.Value.Name, estateResult.Value.Location, estateResult.Value.Price, estateResult.Value.TotalArea, estateResult.Value.Utilities, estateResult.Value.Description, estateResult.Value.Image)
        };
    }
}