...
public class CreateEstateCommandHandler : ICommandHandler<CreateEstateCommand, CreateEstateCommandResponse>
{
    ...
    public async Task<CreateEstateCommandResponse> Handle(CreateEstateCommand command, CancellationToken cancellationToken)
    {
        ...
        var result = Estate.Create(command.EstateData);
        ...
        {
            Success = true,
            Estate = new CreateEstateDto(estate.Id, estate.OwnerId, estate.EstateType.ToString(), estate.EstateCategory.ToString(), estate.Name, estate.Location)
        };
    }
}